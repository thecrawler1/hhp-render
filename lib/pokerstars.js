...
function renderShowdown(down, seatsByPlayer, currency, chips, muckRevealed) {
  const lines = down
  .map(x => {
      if (x.type === 'reveal' && muckRevealed) return `${x.player}: shows ${cards(x)}`
      if (x.type === 'show') return `${x.player}: shows ${cards(x)}${description(x)}`
      if (x.type === 'muck') return `${x.player}: mucks hand`
      if (x.type === 'collect') return `${x.player} collected ${chips(x.amount)} from ${collectPot(x)}`
...
function renderPokerStars(hand, { muckRevealed = true } = {}) {
  const {
    ...
    pokertype, fast, limit, sb, bb, level,
  ...
  const table =
    `Table '` +
    (istourney ? `${gameno} ` : ``) +
    `${tableno}' ${maxseats}-max Seat #${button} is the button\n`
  ...
  for (var i = 0; i < hand.posts.length; i++) {
    var x = hand.posts[i]
    if (x.type != 'post') continue
    x.type = x.amount == bb ? 'bb' : 'sb'
    if (x.amount > bb) {
      x.amount = sb
      hand.posts.splice(i + 1, 0, {
        player: x.player,
        type: 'bb',
        amount: bb
      })
  ...
  const showdown = hand.showdown.length > 0
    ? `\n*** SHOW DOWN ***\n` + renderShowdown(hand.showdown, seatsByPlayer, currency, chips, muckRevealed)
    : ''
